  ...
  <div class="overlay" :class="{ show: show }" @click.self="$emit('close')">
    <div class="modal">
      ...
      <form @submit.prevent="save">
        ...
        <!-- Jadwal Vendor — berlaku buat semua kategori. Rumah data semua
             janji bertanggal dengan vendor (meeting, food testing, survey,
             fitting, gladi bersih, dst). Tiap baris yang punya tanggal
             otomatis muncul di tab Timeline; Timeline cuma baca dari sini. -->
        <div class="vm-section-lbl">🗓 Jadwal Vendor</div>
        <div class="vm-inc-hint">Janji bertanggal dengan vendor ini — meeting, food testing, survey lokasi, fitting, gladi bersih, dan lainnya. Setiap jadwal yang diisi tanggalnya otomatis muncul di tab Timeline.</div>
        <RepeatableCard
          :items="form.jadwal"
          :fields="jadwalFields"
          icon="🗓"
          :title="jadwalTitle"
          :subtitle="jadwalSubtitle"
          :new-item="() => ({ jenis: 'Meeting Vendor', judul: '', tanggal: null, status: 'belum', catatan: '' })"
          add-label="+ Tambah Jadwal"
          empty-label="Belum ada jadwal dengan vendor ini."
        />
...
// Jadwal vendor — dibaca tab Timeline lewat useTimelineFeed(). Field
// `tanggal` sengaja nullable: baris tanpa tanggal tetap boleh disimpan
// (misal jadwal yang belum fix), cuma nggak ikut nongol di garis waktu.
const jadwalFields = [
  {
    key: 'jenis', label: 'Jenis Jadwal', type: 'select', placeholder: 'Pilih jenis',
    options: () => VENDOR_JADWAL_JENIS.map(j => ({ value: j, label: j })),
  },
  { key: 'tanggal', label: 'Tanggal', type: 'date' },
  { key: 'judul', label: 'Judul (opsional)', type: 'text', placeholder: 'cth: Food testing sesi 2', full: true },
  {
    key: 'status', label: 'Status', type: 'select',
    options: () => [{ value: 'belum', label: 'Belum' }, { value: 'selesai', label: 'Selesai' }],
  },
  { key: 'catatan', label: 'Catatan (opsional)', type: 'text', placeholder: 'cth: bawa contoh dekor', full: true },
]
function jadwalTitle(item) {
  return (item.judul || '').trim() || item.jenis || 'Jadwal baru'
}
function jadwalSubtitle(item) {
  const lines = []
  const kapan = item.tanggal ? fmtDate(item.tanggal) : 'Tanggal belum diisi'
  lines.push(`${item.jenis || 'Jadwal'} · ${kapan}${item.status === 'selesai' ? ' · Selesai' : ''}`)
